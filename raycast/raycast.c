...
//
void draw_bound(flame_obj_t *fo, const bound_t *b)
{
  flame_draw_line(fo, b->x1, b->y1, b->x2, b->y2);
}
...
//Line to circle (particle) intersection
static inline int rp_intersect(const ray_t *r, const particle_t *p, double *x1, double *y1, double *x2, double *y2)
{
}
  ...
  for (unsigned i = 0; i < nr; i++)
    {
      /*
	   ...
	   In other terms, the distance between the ray and the intersection point 
	   must be calculated for each intersection. The ray's coordinates
	   will only be updated if the distance is smaller than the current distance. 
