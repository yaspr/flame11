...
//(x, y) is the enter of the rectangle, d is the 
...
int main(int argc, char **argv)
{
  ...
  ppm_t *ppm_file = ppm_open(argv[1]);
 ...
 run:
  ...
  printf("# %d x %d\n", ppm_file->w, ppm_file->h);
  
  draw_frame(fo, ppm_file->w, ppm_file->h);
  
  //ppm_grayscale(ppm_file);
  ppm_draw(fo, ppm_file);
  ...
  while (!done)
      ...
      if (c == 1)
	{
	  ...
	  ppm_zoom_on_click(ppm_file, click_x, click_y);
	...
	if (c == 'q')
	  ...
	  if (c == 'a')
	    ...
	    if (c == 'i')
	      {
		ppm_zoom_in_x2(ppm_file);
		goto run;
	      }
	    else
	      if (c == 'o')
		{
		  ppm_zoom_out_x2(ppm_file);
		  goto run;
		}
	      else
		if (c == 'r')
